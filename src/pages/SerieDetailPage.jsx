...
import { Star, Calendar, Clock, Tv, Film, ShoppingCart, Youtube, X, PlayCircle, Clapperboard } from 'lucide-react';
...
export default function SerieDetailPage() {
  ...
  useEffect(() => {
    const fetchSerieDetails = async () => {
      ...
      try {
        const urls = [
          `https://api.themoviedb.org/3/tv/${serieId}?language=pt-BR`,
          `https://api.themoviedb.org/3/tv/${serieId}/credits?language=pt-BR`,
          `https://api.themoviedb.org/3/tv/${serieId}/watch/providers`,
          `https://api.themoviedb.org/3/tv/${serieId}/videos?language=pt-BR`
        ];
        ...
        if (!detailsResponse.ok) throw new Error('Falha ao buscar detalhes da série.');
  ...
  if (!serie) return <div className="text-center p-10 text-xl dark:text-gray-300">Série não encontrada.</div>;

  const personalRating = personalRatingsMap?.get(serie.id);
  const tmdbRatingColor = getRatingColor(serie.vote_average);
  const personalRatingColor = getRatingColor(personalRating);
  const hasAnyProvider = serie.providers && (serie.providers.flatrate || serie.providers.rent || serie.providers.buy);
      ...
      <div className="animate-fade-in-up">
        <div className="relative w-full overflow-hidden">
          <div className="absolute inset-0">
            <img src={serie.backdrop_path ? `https://image.tmdb.org/t/p/w1280${serie.backdrop_path}` : ''} alt="" className="w-full h-full object-cover opacity-20" />
          ...
          <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="flex flex-col md:flex-row gap-8">
              <div className="md:w-1/3 flex-shrink-0">
                <img src={serie.poster_path ? `https://image.tmdb.org/t/p/w500${serie.poster_path}` : 'https://placehold.co/500x750/374151/FFFFFF?text=Sem+Imagem'} alt={`Pôster de ${serie.name}`} className="rounded-lg shadow-2xl w-full" />
              </div>

              <div className="md:w-2/3 text-gray-800 dark:text-gray-200 flex flex-col gap-8">
                <div>
                  <h2 className="text-4xl md:text-5xl font-black mb-2">{serie.name}</h2>
                  {serie.tagline && <p className="text-lg text-gray-500 dark:text-gray-400 italic mb-6">"{serie.tagline}"</p>}
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm mb-6">
                    <span className={`font-bold flex items-center gap-2 animate-pulse ${tmdbRatingColor}`}><Star size={16} />Nota TMDB: {serie.vote_average.toFixed(1)}</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm mb-6">
                    <div className="flex items-center gap-2"><Calendar size={16} className="text-purple-500" /> <div><strong>Primeiro Episódio:</strong><br/>{new Date(serie.first_air_date).toLocaleDateString('pt-BR')}</div></div>
                    {serie.number_of_seasons && <div className="flex items-center gap-2"><Tv size={16} className="text-purple-500"/><div><strong>Temporadas:</strong><br/>{serie.number_of_seasons}</div></div>}
                    {serie.number_of_episodes && <div className="flex items-center gap-2"><Clapperboard size={16} className="text-purple-500"/><div><strong>Episódios:</strong><br/>{serie.number_of_episodes}</div></div>}
                  </div>
                  <h3 className="text-xl font-bold mb-2">Sinopse</h3>
                  <p className="text-gray-700 dark:text-gray-300 leading-relaxed">{serie.overview}</p>
                </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            ...
            <div>
              ...
              {hasAnyProvider ? (
                <a href={serie.providers.link} target="_blank" rel="noopener noreferrer" className="block bg-white/50 dark:bg-gray-800/50 p-6 rounded-lg hover:ring-2 hover:ring-purple-500 transition-all cursor-pointer">
                  <div className="space-y-4">
                    <ProviderSection title="Streaming" providersList={serie.providers.flatrate} icon={<Tv size={20} />} />
                    <ProviderSection title="Alugar" providersList={serie.providers.rent} icon={<Film size={20} />} />
                    <ProviderSection title="Comprar" providersList={serie.providers.buy} icon={<ShoppingCart size={20} />} />
                  </div>
                </a>
              ) : (
