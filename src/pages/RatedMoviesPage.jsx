...
export default function RatedMoviesPage() {
  const { movies, loading, hasMore, error, lastMovieElementRef } = useMovieFetching(RATED_MOVIES_URL);
  ...
  return (
    <>
      <div className="container mx-auto p-4 sm:p-6 lg:p-8">
        <h1 className="text-4xl font-black text-center mb-10 text-gray-900 dark:text-white">Minhas Avaliações</h1>
        {error && <div className="text-center p-10 text-xl text-red-500">{`Erro: ${error}`}</div>}
        {movies.length === 0 && !loading && <div className="text-center p-10 text-lg text-gray-500">Você ainda não avaliou nenhum filme.</div>}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
